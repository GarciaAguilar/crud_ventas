...
class Venta extends BaseModel {
   ...
   public function crearVenta($idCliente, $productos, $estado = 1) {
    try {
        $this->db->beginTransaction();

        // 1. Insertar la venta principal
        $query = "INSERT INTO ventas (id_cliente, subtotal, impuestos, total, estado) 
                  VALUES (:id_cliente, 0, 0, 0, :estado)";
        ...
        // 3. Calcular impuestos (13% de IVA)
...
public function registrarPago($idVenta, $montoRecibido, $cambio = null) {
    try {
        // Si no se proporciona cambio, calcularlo
        if($cambio === null) {
     ...
     */
    public function obtenerTodasConClientes() {
        try {
            $query = "SELECT v.*, c.nombre as nombre_cliente
                      FROM {$this->table} v
                      LEFT JOIN clientes c ON v.id_cliente = c.id_cliente
            ...
            error_log("Error en Venta::obtenerTodasConClientes: " . $e->getMessage());
            return [];
        }
    }
}
